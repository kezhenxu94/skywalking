...
package org.apache.skywalking.oap.server.receiver.event.handler.grpc;
...
public class EventServiceHandler extends EventServiceGrpc.EventServiceImplBase implements GRPCHandler {
    ...
    public EventServiceHandler(ModuleManager moduleManager) {
        ...
        eventAnalysisService = moduleManager.find(EventModule.NAME)
                                            .provider()
                                            .getService(EventAnalysisService.class);
        ...
        return new StreamObserver<Event>() {
            ...
            public void onNext(final Event event) {
                try (HistogramMetrics.Timer ignored = histogram.createTimer()) {
                    eventAnalysisService.analyze(event);
                } catch (Exception e) {
                    errorCounter.inc();
