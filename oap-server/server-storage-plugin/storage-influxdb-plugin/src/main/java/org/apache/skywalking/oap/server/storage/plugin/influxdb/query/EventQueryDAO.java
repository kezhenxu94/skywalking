...
public class EventQueryDAO implements IEventQueryDAO {
    ...
    public Events queryEvents(final EventQueryCondition condition) throws Exception {
        ...
        final SelectQueryImpl countQuery = select().count(EventRecord.UUID).from(client.getDatabase(), EventRecord.INDEX_NAME);
    ...
    protected Event parseSeriesValues(final QueryResult.Series series, final List<Object> values) {
        ...
        event.setUuid((String) data.get(EventRecord.UUID));

        final String service = (String) data.get(EventRecord.SERVICE);
        final String serviceInstance = (String) data.get(EventRecord.SERVICE_INSTANCE);
        final String endpoint = (String) data.get(EventRecord.ENDPOINT);
        ...
        event.setStartTime(((Number) data.get(EventRecord.START_TIME)).longValue());
        event.setEndTime(((Number) data.get(EventRecord.END_TIME)).longValue());
    ...
    protected WhereQueryImpl<SelectQueryImpl> buildQuery(final EventQueryCondition condition) {
        ...
        final WhereQueryImpl<SelectQueryImpl> query =
            select().raw(ALL_FIELDS)
                    .function(topFunc, EventRecord.START_TIME, condition.getSize())
                    .from(client.getDatabase(), EventRecord.INDEX_NAME)
                    .where();

        if (!isNullOrEmpty(condition.getUuid())) {
            query.and(eq(EventRecord.UUID, condition.getUuid()));
        }

        final Source source = condition.getSource();
        if (source != null) {
            if (!isNullOrEmpty(source.getService())) {
                query.and(eq(EventRecord.SERVICE, source.getService()));
            }
            if (!isNullOrEmpty(source.getServiceInstance())) {
                query.and(eq(EventRecord.SERVICE_INSTANCE, source.getServiceInstance()));
            }
            if (!isNullOrEmpty(source.getEndpoint())) {
                query.and(contains(EventRecord.ENDPOINT, source.getEndpoint().replaceAll("/", "\\\\/")));
            }
        }
        ...
        if (condition.getType() != null) {
            query.and(eq(EventRecord.TYPE, condition.getType().name()));
        }

        final Duration startTime = condition.getTime();
        if (startTime != null) {
            if (startTime.getStartTimestamp() > 0) {
                query.and(gt(EventRecord.START_TIME, startTime.getStartTimestamp()));
            }
            if (startTime.getEndTimestamp() > 0) {
                query.and(lt(EventRecord.END_TIME, startTime.getEndTimestamp()));
            }
        }
