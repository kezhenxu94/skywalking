...
import org.apache.skywalking.oap.meter.analyzer.dsl.HistogramTemporality;
    ...
    public void process() {
        ...
        try {
            converts.forEach(convert -> convert.toMeter(meters.entrySet().stream().collect(toImmutableMap(
                ...
                ).histogramType(HistogramTemporality.DELTA).defaultHistogramBucketUnit(TimeUnit.MILLISECONDS).build()
            ))));
        } catch (Exception e) {
